...
import { estimateObjectSize, generateScanId, parseReportMetadata } from "./history-utils";
...
export class ScanHistoryManager {
	 ...
	 */
	static getByUrl(url: string, exactMatch = false): StorageResult<ScanRecord[]> {
		try {
			const history = this.loadHistory();
